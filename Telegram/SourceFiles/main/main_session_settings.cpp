...
QByteArray SessionSettings::serialize() const {
	...
	size += sizeof(qint32); // _enterpriseAutoBroadcast
	...
	result.reserve(size);
	{
		...
		stream << qint32(_enterpriseAutoBroadcast ? 1 : 0);
	}
...
void SessionSettings::addFromSerialized(const QByteArray &serialized) {
	...
	qint32 enterpriseAutoBroadcast = 0;
	...
	if (!stream.atEnd()) {
		stream >> enterpriseAutoBroadcast;
	}
	if (stream.status() != QDataStream::Ok) {
		LOG(("App Error: "
			"Bad data for SessionSettings::addFromSerialized()"));
	...
	_enterpriseAutoBroadcast = (enterpriseAutoBroadcast == 1);
